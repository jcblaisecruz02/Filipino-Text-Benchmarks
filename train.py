...
# Helper Functions
def tokenize(t): return tokenizer.encode(t, pad_to_max_length=True, max_length=args.msl)
def accuracy(out, y): return (out.argmax(1) == y).sum().item() / len(y)
...
X_train = torch.tensor(np.array([tokenize(t) for t in tqdm(df[args.text_column])]))
...
X_valid = torch.tensor(np.array([tokenize(t) for t in tqdm(df[args.text_column])]))
